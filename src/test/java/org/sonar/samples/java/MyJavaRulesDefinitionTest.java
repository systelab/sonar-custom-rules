...
class MyJavaRulesDefinitionTest {
  ...
  private static void assertParameterProperties(Repository repository) {
     ...
     */
    assertThat(repository.rule("AvoidAssertEquals").params().size()).isEqualTo(0);
  }

  private static void assertRuleProperties(Repository repository) {
    Rule rule = repository.rule("AvoidAssertEquals");
    assertThat(rule).isNotNull();
    assertThat(rule.name()).isEqualTo("Use assertJ equalsThat instead of Junit assertEquals method");
